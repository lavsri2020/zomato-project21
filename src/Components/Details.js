...
class Details extends React.Component {
    ...
    componentDidMount() {
        ...
        axios({
            url: `http://localhost:1910/api/restaurant/${restaurant}`,
            method: 'GET',
    ...
    handleModal = (state, value) => {
        console.log('printing the state values '+JSON.stringify(this.state));
        const { resId } = this.state;
        if (state == "menuItemsModalIsOpen" && value == true) {
            console.log(resId);
            axios({
                url:`http://localhost:1910/api/menuitems/${resId}`,
                   method: 'GET',
                headers: { 'Content-Type': 'application/json' }
            }).then(res => {
    ...
    post = (details) => {
        const form = this.buildForm(details)//form data format
    ...
    getData = (data) => {
        return fetch(`http://localhost:1910/api/payment`, {
            method: "POST",
            headers: {
...
render ()
{
    ...
    return(
        <div>
             <div>
                    <img src={`../${restaurant.image}`} alt="No Image, Sorry for the Inconvinience" width="100%" height="380" />
                ...
                >
                    <div>
                        <div class="fas fa-backspace"style={{ float: 'right', marginBottom: '10px' }} onClick={() => this.handleModal('galleryModalIsOpen', false)}></div>
                        <Carousel
                            showThumbs={false}>
                            {restaurant && restaurant.thumb && restaurant.thumb.map((item) => {
                                return <div>
                                    <img src={`../${item}`} width="50%"height="450px" />
                                </div>
                            })}
                        </Carousel>
                    </div>
                ...
                >
                    <div>
                        ...
                        <div className="menuitems-resize" >
